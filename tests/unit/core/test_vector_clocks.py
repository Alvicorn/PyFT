...
class TestBasicAccessors:
    ...
    def test_increment_increases(self):
        vc = VectorClock({1: 3})
        new_val = vc.increment(1)
        assert new_val == 4
        assert vc.get(1) == 4
...
class TestHappensBefore:
    ...
    def test_vc_leq_missing_entries_are_zero(self):
        a = VectorClock({1: 1})
...
class TestMerging:
    ...
    def test_joined_static_method(self):
        a = VectorClock({1: 5})
...
class TestEpochConversions:
    ...
    def test_add_epoch_updates_if_newer(self):
        vc = VectorClock({1: 3})
        vc.add_epoch(1, 5)
        assert vc.get(1) == 5

    def test_add_epoch_ignores_older(self):
        vc = VectorClock({1: 5})
        vc.add_epoch(1, 3)
        assert vc.get(1) == 5
...
class TestEquality:
    ...
    def test_missing_and_zero_are_equal(self):
        a = VectorClock({1: 0})
        b = VectorClock()
        assert a == b
