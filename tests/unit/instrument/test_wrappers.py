...
class TestTrackedProxy:
    def test_proxy_forwards_getattr(self):
        engine = Engine()
        c = Counter(42)
        proxy = TrackedProxy(c, engine)
        assert proxy.value == 42

    def test_proxy_forwards_setattr(self):
        engine = Engine()
        c = Counter(0)
        proxy = TrackedProxy(c, engine)
        proxy.value = 99
    ...
    def test_read_calls_engine_read(self):
        engine = Engine()
        calls = []
    ...
    def test_write_calls_engine_write(self):
        engine = Engine()
        calls = []
    ...
    def test_internal_attrs_not_tracked(self):
    ...
    def test_repr_includes_wrapped_repr(self):
        engine = Engine()
        c = Counter(7)
        proxy = TrackedProxy(c, engine)
        assert "TrackedProxy" in repr(proxy)

    def test_proxy_is_transparent_for_method_calls(self):
        engine = Engine()
...
class TestMakeTrackedClass:
    def test_tracked_class_calls_engine_on_setattr(self):
        engine = Engine()
        writes = []
    ...
    def test_tracked_class_calls_engine_on_getattr(self):
        engine = Engine()
        reads = []
        original_read = engine.read
        engine.read = lambda obj, attr: (
    ...
    def test_tracked_class_is_subclass(self):
        engine = Engine()
        TrackedCounter = make_tracked_class(Counter, engine)
        c = TrackedCounter()
        assert isinstance(c, Counter)

    def test_tracked_class_name_includes_original(self):
        engine = Engine()
        TrackedCounter = make_tracked_class(Counter, engine)
...
class TestAutoTracker:
    def test_install_patches_thread(self):
        engine = Engine()
    ...
    def test_uninstall_restores_thread(self):
        engine = Engine()
        tracker = AutoTracker(engine)
        tracker.install()
        tracker.uninstall()
    ...
    def test_thread_registered_on_start(self):
        engine = Engine()
        tracker = AutoTracker(engine)
        tracker.install()
