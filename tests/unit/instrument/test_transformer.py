...
from pyvft.instrument.transformer import AccessTracer, should_skip_module


class TestShouldSkipModule:
    def test_pyvft_skipped(self) -> None:
        assert should_skip_module("pyvft")
        assert should_skip_module("pyvft.core.var_state")
...
class TestTransformSource:
    ...
    def test_attribute_read_becomes_helper_call(self) -> None:
        ...
        assert ret.value.func.id == "__pyvft_get__"
    ...
    def test_attribute_write_becomes_helper_call(self) -> None:
        ...
        assert stmt.value.func.id == "__pyvft_set__"
    ...
    def test_subscript_read(self) -> None:
        ...
        ret = fn.body[0]
        assert isinstance(ret.value, ast.Call)
        assert ret.value.func.id == "__pyvft_getitem__"
    ...
    def test_subscript_write(self) -> None:
        ...
        stmt = fn.body[0]
        assert isinstance(stmt, ast.Expr)
        assert stmt.value.func.id == "__pyvft_setitem__"
    ...
    def test_augmented_assignment_does_both(self) -> None:
        ...
        stmt = fn.body[0]
        assert isinstance(stmt, ast.Expr)
        # outer is __pyvft_set__, inner has __pyvft_get__
        assert stmt.value.func.id == "__pyvft_set__"
        # third arg is the BinOp whose left is __pyvft_get__ call
        bin_op = stmt.value.args[3]
        assert isinstance(bin_op, ast.BinOp)
        assert isinstance(bin_op.left, ast.Call)
        assert bin_op.left.func.id == "__pyvft_get__"
...
class TestRuntimeModule:
    def test_get_calls_engine_read_and_returns_value(self) -> None:
        ...
        obj = C()
        result = rt._pyvft_get(engine, obj, "x")
        assert result == 7
        assert reads == ["x"]
    ...
    def test_get_swallows_engine_errors(self) -> None:
        ...
        assert rt._pyvft_get(engine, C(), "v") == 42
...
class TestImportHookEndToEnd:
    ...
    def _setup(self, tmp_path: Path) -> Path:
        ...
        fixture = tmp_path / "pyvft_fixture_module.py"
    ...
    def _teardown(self, tmp_path: Path) -> None:
        ...
        sys.modules.pop("pyvft_fixture_module", None)
    ...
    def test_import_hook_traces_attribute_access(self, tmp_path: str) -> None:
        ...
        try:
            ...
            hook.install()
            try:
                import pyvft_fixture_module as m
